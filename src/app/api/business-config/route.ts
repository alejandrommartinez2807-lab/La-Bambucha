...
function normalizeBusinessConfigPayload(
  ...
  if (hasOwn(source, "businessName")) {
    config.businessName = readString(source, "businessName")
  }

  if (hasOwn(source, "businessShortDescription")) {
    config.businessShortDescription = readString(source, "businessShortDescription")
  }
  ...
  if (hasOwn(source, "mainWhatsapp")) {
    config.mainWhatsapp = readString(source, "mainWhatsapp")
  }

  if (hasOwn(source, "deliveryWhatsapp")) {
    config.deliveryWhatsapp = readString(source, "deliveryWhatsapp")
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const rawBusinessConfig = (body.businessConfig || body.config || body || {}) as Record<
      ...
      unknown
    >
    const currentBusinessConfig = (await getBusinessConfigFromAppsScript()) as unknown as Record<
      string,
      unknown
    >
    ...
    const businessConfig = await saveBusinessConfigInAppsScript(businessConfigInput)
