...
export async function GET(request: NextRequest) {
  try {
    ...
    const dayExpenses = await getDayExpensesFromAppsScript({
      ...(dateFrom ? { dateFrom } : {}),
      ...(dateTo ? { dateTo } : {}),
      ...(dateValue ? { dateValue } : {}),
    })
...
export async function POST(request: NextRequest) {
  try {
    ...
    const dayExpense = await saveDayExpenseInAppsScript(input)
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    await deleteDayExpenseInAppsScript(expenseId)
