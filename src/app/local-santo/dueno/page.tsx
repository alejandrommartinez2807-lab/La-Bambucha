        ...
        <section className="sticky top-0 z-30 mt-4 rounded-[1.4rem] border-2 border-[#a00000] bg-white p-3 shadow-[0_8px_0_rgba(160,0,0,0.10)]">
          ...
          {areControlsVisible && (
            <>
              ...
              {rangeMode === "custom" && (
                <div className="mt-3 grid gap-3 rounded-[1.2rem] border border-[#a00000]/20 bg-[#fff7e8] p-3 sm:grid-cols-2">
                  <DateFilterInput
                    label="Desde"
  ...
  return (
    <label className="block">
      ...
      <input
        ...
        className="mt-2 w-full rounded-2xl border-2 border-[#a00000]/25 bg-white px-4 py-3 text-sm font-black text-[#4a0000] outline-none focus:border-[#a00000]"
      />
    </label>
  )
}
...
function BigMetricCard({
  ...
  const toneClass =
    tone === "good"
      ...
      : tone === "warning"
        ? "border-yellow-400 bg-yellow-100"
        : "border-[#a00000] bg-white"

  return (
    <article className={`rounded-[1.5rem] border-2 p-4 shadow-[0_8px_0_rgba(160,0,0,0.10)] ${toneClass}`}>
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          ...
          <p className="mt-2 break-words text-3xl font-black leading-none text-[#220000]">
            {mainValue}
          </p>
          ...
          {items.map((item, index) => (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="break-words text-sm font-black uppercase text-[#220000]">
                    {index + 1}. {item.label}
                  </p>
...
function QuickReadLine({
  ...
  return (
    <div className="rounded-2xl border border-[#a00000]/15 bg-[#fff7e8] p-3">
      <div className="flex gap-3">
        <div className="mt-0.5 text-[#a00000]">{icon}</div>
        <div>
          <p className="text-[0.68rem] font-black uppercase tracking-[0.14em] text-[#a00000]">
            {label}
          </p>
          <p className="mt-1 text-sm font-bold leading-5 text-[#220000]">
            {value}
          </p>
        </div>
      </div>
...
function AttentionOrderCard({ order }: { order: LocalOrder }) {
  ...
  return (
    <article className="rounded-[1.4rem] border-2 border-[#a00000]/25 bg-[#fff7e8] p-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <span className="inline-flex items-center gap-2 rounded-full border-2 border-[#a00000] bg-white px-3 py-1 text-xs font-black uppercase text-[#a00000]">
              {statusIcon}
              {getDisplayOrderNumber(order)}
            </span>

            <span className="rounded-full border-2 border-[#a00000]/25 bg-white px-3 py-1 text-xs font-black uppercase text-[#3a0000]">
              {payment.status}
            </span>

            {isReportedDelivery && (
              <span className="rounded-full border-2 border-yellow-400 bg-yellow-100 px-3 py-1 text-xs font-black uppercase text-[#8a5a00]">
                Delivery reportado
              </span>
            )}
          </div>

          <h3 className="mt-3 text-xl font-black uppercase text-[#220000]">
        ...
        <div className="rounded-2xl border-2 border-[#a00000] bg-white px-4 py-3 text-right">
          <p className="text-xs font-black uppercase tracking-[0.14em] text-[#a00000]">
            Total
          </p>
          <p className="mt-1 text-xl font-black text-[#220000]">
