...
export default function FrequentCustomersPage() {
  ...
  if (!adminPassword) {
    return (
      <main className="min-h-screen bg-[#fff7e8] px-4 py-6 text-[#220000]">
        <section className="mx-auto max-w-xl rounded-[1.5rem] border-4 border-[#a00000] bg-white p-5 shadow-[0_10px_0_rgba(160,0,0,0.12)]">
          <a
            href="/local-santo"
          ...
          <div className="mt-5 flex h-14 w-14 items-center justify-center rounded-2xl border-2 border-[#a00000] bg-yellow-300 text-[#4a0000]">
            <UserRound size={28} />
          </div>

          <p className="mt-5 text-xs font-black uppercase tracking-[0.28em] text-[#a00000]">
            Acceso privado
          </p>
          ...
          <div className="mt-5 grid gap-3">
            <input
              ...
              className="rounded-2xl border-2 border-[#a00000]/30 bg-[#fff7e8] px-4 py-3 text-sm font-black outline-none focus:border-[#a00000]"
            ...
            <button
              ...
              className="inline-flex items-center justify-center gap-2 rounded-full border-2 border-[#a00000] bg-yellow-300 px-5 py-3 text-xs font-black uppercase tracking-[0.14em] text-[#4a0000] transition hover:bg-yellow-200 disabled:opacity-50"
            >
  ...
  return (
    <main className="min-h-screen bg-[#fff7e8] px-3 py-4 text-[#220000] sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl">
        ...
        {!canUseCustomers && (
          <section className="mt-4 rounded-[1.4rem] border-2 border-yellow-400 bg-yellow-100 p-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-black uppercase tracking-[0.16em] text-[#8a5a00]">
                  Módulo no activo
                </p>
        ...
        {canUseCustomers && (
          <>
            <section className="sticky top-0 z-20 mt-4 rounded-[1.4rem] border-2 border-[#a00000] bg-white p-3 shadow-[0_8px_0_rgba(160,0,0,0.10)]">
              <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                ...
                <label className="flex min-w-0 items-center gap-2 rounded-full border-2 border-[#a00000]/25 bg-[#fff7e8] px-4 py-2">
                  <Search size={16} className="text-[#a00000]" />
                  <input
                    value={searchText}
                    onChange={(event) => setSearchText(event.target.value)}
                    placeholder="Buscar por nombre, teléfono o producto"
                    className="min-w-0 flex-1 bg-transparent text-sm font-black text-[#3a0000] outline-none placeholder:text-[#3a0000]/45"
                  />
                </label>
              </div>
            </section>
            ...
            {topCustomer && (
              <section className="mt-4 rounded-[1.6rem] border-4 border-[#a00000] bg-yellow-300 p-4 text-[#4a0000] shadow-[0_10px_0_rgba(160,0,0,0.12)]">
                <div className="grid gap-4 lg:grid-cols-[1fr_auto] lg:items-center">
                  ...
                  {buildWhatsAppUrl(topCustomer) ? (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="inline-flex items-center justify-center gap-2 rounded-full border-2 border-[#a00000] bg-white px-5 py-3 text-xs font-black uppercase tracking-[0.14em] text-[#a00000] transition hover:bg-[#fff7e8]"
                    >
                      <MessageCircle size={16} />
                      Escribir por WhatsApp
                    </a>
                  ) : (
                    <span className="rounded-full border-2 border-[#a00000]/40 bg-white/50 px-5 py-3 text-center text-xs font-black uppercase tracking-[0.14em]">
                      Sin WhatsApp válido
                    </span>
                  )}
                </div>
            ...
            <section className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
              {filteredCustomers.map((customer, index) => {
                ...
                return (
                  <article
                    key={customer.key}
                    className="rounded-[1.5rem] border-2 border-[#a00000] bg-white p-4 shadow-[0_7px_0_rgba(160,0,0,0.10)]"
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex h-12 w-12 items-center justify-center rounded-2xl border-2 border-[#a00000] bg-yellow-300 text-[#4a0000]">
                        {index === 0 ? <Star size={22} /> : <UserRound size={22} />}
                      </div>

                      <span className="rounded-full border-2 border-[#a00000]/25 bg-[#fff7e8] px-3 py-1 text-[0.58rem] font-black uppercase tracking-[0.14em] text-[#a00000]">
                        {customer.orderCount} pedido(s)
                      </span>
                    </div>

                    <h2 className="mt-4 text-2xl font-black uppercase leading-none text-[#a00000] drop-shadow-[0_2px_0_rgba(255,211,0,0.75)]">
                      {customer.name}
                    </h2>

                    <div className="mt-3 grid gap-2 text-sm font-bold text-[#3a0000]/75">
                      ...
                      <p className="flex items-center gap-2">
                        <Clock size={16} className="text-[#a00000]" />
                        Último pedido: {formatDate(customer.lastOrderAt)}
                      </p>
                    </div>

                    <div className="mt-4 rounded-2xl border-2 border-[#a00000]/15 bg-[#fff7e8] p-3">
                      <p className="text-[0.62rem] font-black uppercase tracking-[0.18em] text-[#a00000]">
                        Preferencias
                      </p>
                      <p className="mt-1 text-sm font-black text-[#3a0000]">
                      ...
                      <div className="mt-2 flex flex-wrap gap-2">
                        {customer.products.length ? (
                          customer.products.map((product) => (
                            <span
                              key={product.name}
                              className="rounded-full border border-[#a00000]/20 bg-white px-3 py-1 text-[0.68rem] font-black text-[#a00000]"
                            >
                              {product.name} x{product.quantity}
                            </span>
                          ))
                        ) : (
                          <span className="text-xs font-bold text-[#3a0000]/60">
                    ...
                    {whatsappUrl ? (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-full border-2 border-[#a00000] bg-yellow-300 px-4 py-3 text-xs font-black uppercase tracking-[0.14em] text-[#4a0000] transition hover:bg-yellow-200"
                      >
                        <MessageCircle size={16} />
                        Escribir por WhatsApp
                      </a>
                    ) : (
                      <div className="mt-4 rounded-full border-2 border-[#a00000]/25 bg-[#fff7e8] px-4 py-3 text-center text-xs font-black uppercase tracking-[0.14em] text-[#a00000]/70">
            ...
            {!filteredCustomers.length && (
              <section className="mt-4 rounded-[1.4rem] border-2 border-[#a00000]/20 bg-white p-5 text-center">
                <p className="text-sm font-black uppercase tracking-[0.18em] text-[#a00000]">
                  Sin clientes para mostrar
                </p>
                <p className="mt-2 text-sm font-bold text-[#3a0000]/65">
    ...
    tone === "good"
      ...
      : tone === "warning"
        ...
        : tone === "yellow"
          ...
          : "border-[#a00000]/20 bg-[#fff7e8] text-[#a00000]"
